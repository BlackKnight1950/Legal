...
// Diagnostic: returns attachment metadata + whether the server can reach
// storage, as plain JSON. Open in a browser to pinpoint download failures.
// GET /api/attachment-check/:sheet/:attachmentId
...
app.get("/api/attachment/:sheet/:attachmentId", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // The server couldn't reach Smartsheet's storage host (S3). On Render this
      // usually means outbound egress is blocked for that host.
    ...
    // Use the ACTUAL downloaded byte count. Smartsheet's sizeInKb is rounded and
    // does NOT match the real body length — sending it as Content-Length makes
    // the browser abort the transfer as a "network error" on size mismatch.
